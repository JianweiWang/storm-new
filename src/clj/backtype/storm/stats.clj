...
;;added by wjw 2014-06-03
(defrecord BoltExecutorStats [common acked failed process-latencies executed execute-latencies disruptor-usage])
;;end added by wjw
...
;;added by wjw 2014-06-03在BoltExecutorStats中加入disruptor的使用率属性，初始化为0
(defn mk-bolt-stats [rate]
  (BoltExecutorStats. (mk-common-stats rate)
                  (atom (apply keyed-counter-rolling-window-set NUM-STAT-BUCKETS STAT-BUCKETS))
                  (atom (apply keyed-counter-rolling-window-set NUM-STAT-BUCKETS STAT-BUCKETS))
...
;(defn set-disruptor-usage [^BoltExecutorStats stats disruptor] (.getUsage disruptor) )
;;end added by wjw
