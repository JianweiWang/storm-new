...
;;added by wjw 2014-05-13 13:50
;(defn -main []
; (let [t (Test.)]
;   (.run t)))

;;added by wjw 2014-05-14
;;function: 定期获取executor的capacity
;(defn get-executor-capacity [^ExecutorSummary e]
;  (
;    (
;      (while (zero? 0)
;        (Thread/sleep 1000)
;        (let [result (compute-executor-capacity e)]
;          (println result))
;        )
;      )
;  )
...
;(defn get-executor-capacity-thread []
;  (let [sm (StormMonitor.)
;        e (.getExecutor sm)]
;    (
;      (println (class e))
;      (.start (Thread. #(get-executor-capacity e)))
;      )
;    )
;  )
...
(defn get-executor-capacity-thread []
      (.start (Thread. #(get-executor-capacity)))
    
  )

(defn -main [] 
  ;(get-executor-capacity-thread)
  (start-server!)
  )
;;end added by wjw
